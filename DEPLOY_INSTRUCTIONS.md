...
2. Run the deploy script (replace IP and User):
   ...
   ```
   - Enter the Pi password when prompted (if SSH keys are not configured).
3. This script:
   - Resolves the remote home directory (`~/signage-player`) so it is username-agnostic.
   - Copies player files to the Pi.
   - Preserves the remote `config.json` if it already exists (to keep the current pairing), unless you use `-ForceConfigSync`.
   - Copies local `player/config.json` only when the remote config does not exist yet (or when forcing config sync).
...
2. **IMPORTANT: Configure Server URL & Identity**
   ...
   ```
   Ensure the content looks like this (replace URL if needed, but ensure `device_token` is empty for a new pairing):
   ```json
   {
   ...
   ```
   - For a new pairing code, leave `device_token` empty (recommended: `""`).
   - `null` may also work in some flows, but the current scripts generate an empty string.
